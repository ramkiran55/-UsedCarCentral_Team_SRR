...
(
    ...
    VehicleIdentificationNum NVARCHAR(50),
    DriveType NVARCHAR(50),
	FOREIGN KEY (ID) REFERENCES test.UsedCarsMasterData(ID)
);
...
INSERT INTO test.CarsMasterData (ID, Manufacturer, ModelYear, CylinderCount, FuelType, TransmissionType, CarSize -- Revanth Posina (rposina)
...
INNER JOIN test.CarsMasterData c ON ucmd.ID = c.ID;


INSERT INTO test.Locations (ID, City, StateCode, Latitude, Longitude) -- Syam Prajwal Kammula (skammul)
SELECT ID, City, StateCode, Latitude, Longitude
FROM test.UsedCarsMasterData;


INSERT INTO test.CarListings (ID, CarID, LocationID, Price, PostedDate, ListingURL) -- Ram Kiran Devireddy (radevir)
SELECT ucmd.ID, c.CarID, l.LocationID, Price, PostedDate, ListingURL
FROM test.UsedCarsMasterData ucmd
INNER JOIN test.CarsMasterData c ON ucmd.ID = c.ID
INNER JOIN test.Locations l ON ucmd.ID = l.ID;
